...
def _get_labels_from_crop_indexes(
    crop_indexes: List[CropIndex]
) -> np.ndarray:
    # [1, n_gallery]
    person_ids = np.array(crop_indexes.keys()).reshape(1, -1)
...
def evaluate_from_ground_truth(
    ...
    gallery_features = np.concatenate([
        crop_index_to_crop_features[crop_index]
        for crop_index in crop_indexes
    ])

    all_similarities = np.einsum(
        'nd,md->n,m', queries_features, gallery_features)

    all_labels = _get_labels_from_crop_indexes(crop_indexes)

    return np.mean([
        compute_average_precision(similarities, labels)
