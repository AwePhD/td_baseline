...
from .metrics import compute_mean_average_precision
...
def evaluate_treid_from_h5(
    ...
    labels = (person_ids.reshape(-1, 1) == person_ids.reshape(1, -1)).repeat(
        2, axis=0
    )
    ...
    similarities = np.einsum("nd,md->nm", features_query, features_gallery)
